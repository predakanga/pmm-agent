...
func workLoop(ctx context.Context, cfg *config.Config, client agent.AgentClient) {
	...
	agentMessage := &agent.AgentMessage{
		Id: id,
		Payload: &agent.AgentMessage_Connect{
			Connect: &agent.ConnectRequest{
				Uuid:    cfg.UUID,
				Version: Version,
			},
		},
	...
	for {
		...
		case *agent.ServerMessage_State:
			for _, exporter := range payload.State.MysqldExporters {
...
func main() {
	...
	logrus.Infof("Loaded configuration: %s.", cfg)
	...
	if cfg.Address == "" {
		logrus.Error("PMM Server address is not provided, blocking.")
		<-ctx.Done()
		return
	}
	...
	if true { // FIXME add flag that defaults to false
		opts = append(opts, grpc.WithInsecure())
	}
	// TODO grpc.WithTransportCredentials() / grpc.WithPerRPCCredentials() / grpc.WithAuthority()
